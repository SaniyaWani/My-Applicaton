...
export default class HomeScreen extends Component {
    ...
    getTime = async () => {
        ...
        await fetch('http://api.aladhan.com/v1/calendar?latitude=20.5937&longitude=78.9629&method=2&month=' + month + '&year=' + year + '')
            ...
            .then(data => {
                console.log(data.data[date - 1].date.hijri)
                this.setState({ 
                    fechtedInfo:data.data[date - 1].date.hijri,
    ...
    render() {
        ...
        return (
            ...
            <View style={styles.whole}>
                <View style={styles.textstyle,{alignItems:'center',flexDirection: 'row'}}>
                <Icon name="arrow-left" size={20} color="black" type="entypo"  size={35}
                onPress={() =>this.props.navigation.navigate('Lastdaystime')}/>
                    <Text style={{ fontSize: 40 }}>Today's Time</Text>
                    <Icon name="arrow-right" size={20} color="black" type="entypo" size={35}
                       onPress={() =>this.props.navigation.navigate('Nextdaystime')}/>
